...
// and someNormals parameters must both be valid pointers to at 
...
// each coordinate has size (2 * GLfloat). If not NULL, 
// someIndices must be a valid pointer to countIndices vertex 
// indices where each index has size (1 * GLushort).
...
- (void)prepareToDraw;
{
   ...
   {  // vertex attiributes haven't been sent to GPU yet
      self.vertexAttributeBuffer = 
         ...
         numberOfVertices:(GLsizei)([self.vertexData length] /
